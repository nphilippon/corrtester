...
The shiny app has been containerized with Docker and is readily available for pulling via DockerHub.

### Building Image Locally

In the case that you are interested on developing this app further see the instructions below to build locally.
...
GitHub actions workflow is setup so when someone pushes to the GitHub it automatically builds and pushes the image to DockerHub.

This pipeline includes the following:

-   Uses docker actions which include checkout, buildx, login all with their most recent versions
-   Authenticates DockerHub via Secrets including login username and secret credential
-   Has cache set up so it doesn't waste time building the entire app every time it's loaded
...
For ARM cpu users (Silicon Mac commonly) ensure after pulling you use this run command instead, for emulation.

``` terminal
docker run --platform linux/amd64 -p 3838:3838 cainaidoo/corrtester:latest
```

Then go to the following website: - <http://localhost:3838>
...
The container supports the following features: - Multi CPU support (Does not run natively on ARM, base image could only work on AMD) - Image versions via tagging (`latest` and `${{github.run_number}}`) - Health monitoring to ensure shiny app is responsive via `HEALTHCHECK` - Cloud deployment ready without additional code (Via AWS, Azure, Google)
...
-   Try to see in the future if we can have it so ARM users can run the container the normal way AMD users do without getting the following warning below
